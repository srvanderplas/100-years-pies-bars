...
chart_data <- data.frame(
  Chart_Type = rep(c("Bar", "Pie", "Donut"), each = 6),
  Task_Type = rep(c("A vs B", "A / (A + B)", "A vs (B + C)", 
                    "(A + B) vs (C + D)", "Ranking", "Magnitude Est."), 3),
  Rank = c(
    1, 3, 1, 2, 1, 3,  # Bar
    2, 1, 2, 1, 2, 1,  # Pie
    3, 2, 3, 3, 3, 1   # Donut
...
# Reorder x-axis: Pie, Bar, Donut
chart_data$Chart_Type <- factor(chart_data$Chart_Type, levels = c("Pie", "Bar", "Donut"))

# Set Rank so that 1 shows on top
chart_data$Rank <- factor(chart_data$Rank, levels = c(3, 2, 1))

# Plot
ggplot(chart_data, aes(x = Chart_Type, y = Rank, fill = Task_Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  labs(
    title = "Chart Type Performance Across Tasks",
    y = "Rank (1 = Best)",
    x = NULL,
    fill = "Task Type"
  ) +
...
df2 <- data.frame(
  ...
  sub_group = rep(c("A", "B", "C", "D"), 1),
  value = c(20, 30, 25, 25)
)

df2$facet <- rep(c("Group AB", "Group CD"), each = 2)

ggplot(df2, aes(x = "", y = value, fill = sub_group)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y") +
  facet_wrap(~ facet) +
  theme_void() +
  ggtitle("Comparison: AB vs CD") +
  theme(
...
make_pie_data <- function(labels, values, group) {
  df <- data.frame(
    label = labels,
    value = values
  )
...
# Create all pie segments
pie_data <- bind_rows(
  make_pie_data(c("A", "B"), c(0.4, 0.6), "A vs B"),
  make_pie_data(c("A", "B", "C"), c(0.25, 0.25, 0.5), "A vs (B+C)"),
  make_pie_data(c("A+B", "C"), c(0.5, 0.5), "(A+B) vs C")
)

# Prepare for pie chart layout
pie_data <- pie_data %>%
  group_by(group) %>%
  mutate(
...
# Plot
ggplot(pie_data, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = label)) +
  geom_rect(color = "white") +
  coord_polar(theta = "y") +
  facet_wrap(~group, ncol = 1, strip.position = "left") +
  geom_text(aes(x = 0.5, y = label_pos, label = label), size = 5) +
  scale_fill_manual(values = c("A" = "#e0f3db", "B" = "#ccebc5", "C" = "#b2df8a", "A+B" = "#a6cee3")) +
  theme_void() +
  theme(
  ...
  ggtitle("Comparison Tasks")

# ==== 1. Full Pie Chart: A, B, C, D ====
df_full <- data.frame(
  group = c("A", "B", "C", "D"),
  value = c(25, 25, 25, 25)
)
...
ggplot(df_full, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = group)) +
  geom_rect(color = "white") +
  geom_text(aes(x = 0.5, y = label_pos, label = group), size = 6, fontface = "bold") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = gray.colors(4, start = 0.9, end = 0.4)) +
  theme_void() +
  ggtitle("Full Pie Chart: A, B, C, D")
...
# Color palette (match original soft hues)
color_palette <- c("A" = "#FBB4AE", "B" = "#B3CDE3", "C" = "#DECBE4", "A+B" = "#CCEBC5")

# Plot
ggplot(pie_data, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = label)) +
  geom_rect(color = "white") +
  geom_text(aes(x = 0.5, y = label_pos, label = label), size = 5) +  # Labels inside
  coord_polar(theta = "y") +
  facet_wrap(~group, nrow = 1) +  # Horizontal layout
...
p_ab <- ggplot(df_ab, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = group)) +
  geom_rect(color = "white") +
  geom_text(aes(x = 0.5, y = label_pos, label = group), size = 6, fontface = "bold") +
  coord_polar(theta = "y", start = pi) +  # start at top to show half-pie
  scale_fill_manual(values = gray.colors(2, start = 0.9, end = 0.7)) +
  theme_void() +
  ggtitle("Group AB")

# ==== 3. Half Pie Chart: Group CD ====
df_cd <- data.frame(
  group = c("C", "D"),
  value = c(50, 50)
)
...
p_cd <- ggplot(df_cd, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = group)) +
  geom_rect(color = "white") +
  geom_text(aes(x = 0.5, y = label_pos, label = group), size = 6, fontface = "bold") +
  coord_polar(theta = "y", start = pi) +
  scale_fill_manual(values = gray.colors(2, start = 0.6, end = 0.4)) +
  theme_void() +
  ggtitle("Group CD")

# ==== Combine AB and CD side by side ====
library(patchwork)
p_ab + p_cd
```
...
# Function to make pie chart with highlighted segments
make_pie_plot <- function(data, title, highlight_group = NULL, highlight_color = "black") {
  data <- data %>%
    mutate(
      fraction = value / sum(value),
      ymax = cumsum(fraction),
  ...
  # Add border flag
  data$highlight <- ifelse(data$label %in% highlight_group, TRUE, FALSE)

  ggplot(data) +
    geom_rect(aes(
      ymin = ymin, ymax = ymax, xmin = 0.5, xmax = 1.5,
      fill = label, color = highlight
    ), size = 1) +
    coord_polar(theta = "y") +
    geom_text(aes(
      x = 1, y = label_pos, label = label
    ), size = 5) +
    scale_fill_manual(values = c(
      "A" = "#f4a582", "B" = "#92c5de", "C" = "#d1bce3", "A+B" = "#b2df8a", "B+C" = "#b2abd2"
    )) +
    scale_color_manual(values = c("TRUE" = highlight_color, "FALSE" = NA), guide = "none") +
    theme_void() +
    ggtitle(title) +
...
data1 <- tibble(label = c("A+B", "C"), value = c(0.5, 0.5))  # (A+B) vs C
data2 <- tibble(label = c("A", "B", "C"), value = c(0.25, 0.25, 0.5))  # A vs (B+C)
data3 <- tibble(label = c("A", "B"), value = c(0.4, 0.6))  # A vs B

# Make each plot with outlines
p1 <- make_pie_plot(data1, "(A+B) vs C", highlight_group = "A+B")
p2 <- make_pie_plot(data2, "A vs (B+C)", highlight_group = c("B", "C"))
p3 <- make_pie_plot(data3, "A vs B", highlight_group = c("A", "B"))
